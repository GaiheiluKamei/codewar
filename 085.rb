...
Description:
  ...
  You can immediately see that going "NORTH" and immediately "SOUTH" is not reasonable, better
  stay to the same place! So the task is to give to the man a simplified version of the plan.
...
def dirReduc(arr)
  ...
  while true
    filter_arr = arr.chunk_while { |i, j| directions[i] == j || directions[j] == i }.reject { |e| e.size == 2 }
    flatten_filter_arr = filter_arr.flatten

    if arr.size == flatten_filter_arr.size
      if filter_arr.any? { |e| e.size == 3 }
        arr = filter_arr.select.map { |e| e.size == 3 ? [e.last] : e }.flatten
      else
        break
      end
    else
      arr = flatten_filter_arr
    end
  end
